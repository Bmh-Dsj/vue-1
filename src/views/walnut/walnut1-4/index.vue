...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      //柱一
      hisOne(){
        this.hisProcessed({allocateTime: this.timeOfDay[0], allocateTime2: this.timeOfDay[1], sourceChannel: ""}).then(res=>{
          let rows = []
          for (let a = 0; a < res.data.x.length ; a++){
            if(res.data.x[a+1]){
              let row = {}
      ...
      //柱二
      hisTwo(){
        this.hisProcessed({allocateTime: this.timeValue1[0], allocateTime2: this.timeValue1[1], sourceChannel: ""}).then(res=>{
          let rows = []
          for (let a = 0; a < res.data.x.length ; a++){
            if(res.data.x[a+1]){
              let row = {}
      ...
      //柱三
      hisThree(){
        this.hisProcessed({allocateTime: this.timeValue4[0], allocateTime2: this.timeValue4[1], sourceChannel: this.value1}).then(res=>{
          let rows = []
          for (let a = 0; a < res.data.x.length ; a++){
            if(res.data.x[a+1]){
              let row = {}
      ...
      //饼一
      pieOne(){
        this.pieDetail({allocateTime: this.timeOfDay[0], allocateTime2: this.timeOfDay[1], sourceChannel: ""}).then(res=>{
          let rows = []
          for (let a = 0; a < res.data.y.length ; a++){
            let row = {}
            row.接通状态 = res.data.x[0][a]
      ...
      //饼二
      pieTwo(){
        this.pieDetail({allocateTime: this.timeValue2[0], allocateTime2: this.timeValue2[1], sourceChannel: ""}).then(res=>{
          let rows = []
          for (let a = 0; a < res.data.y.length ; a++){
            let row = {}
            row.接通状态 = res.data.x[0][a]
      ...
      //饼三
      pieThree(){
        this.pieDetail({allocateTime:this.timeValue5[0], allocateTime2: this.timeValue5[1], sourceChannel: this.value2}).then(res=>{
          let rows = []
          for (let a = 0; a < res.data.y.length ; a++){
            let row = {}
            row.接通状态 = res.data.x[0][a]
      ...
      //比较柱一
      hisFour(){
        this.hisSeats({allocateTime: this.timeOfDay[0], allocateTime2: this.timeOfDay[1], sourceChannel: ""}).then(res=>{
          let rows = []
          for (let a = 0; a < res.data.length ; a++){
            let row = {}
            row.坐席号 = res.data[a].坐席
