  ...
  methods: {
    ...
    blackList(){
      this.$confirm('此操作无法撤销, 是否继续?', '提示', {
        ...
        axios({
          method: 'get',
          url: `${this.baseURL}/jz/admin/adddatabyphoneanddepartguid/${this.onedata.phone}`,
          headers: {
    ...
    dphone(){
      if(this.bddh == '拨打电话'){
        axios({
          method: 'get',
          url: `${this.baseURL}/freeswitch/callPhone/${this.jobNumber}/${this.phone}`,
          headers: {
            "Content-Type": "application/json;",
            'token': localStorage.token,
          },
    ...
    getlist(data){
      ...
      axios({
        method: 'post',
        url: `${this.baseURL}/jz/admin/getneedhandledata`,
        headers: {
          "Content-Type": "application/json;",
          'token': localStorage.token,
        },
    ...
    get(data){
      ...
      // axios({
      //   method: 'get',
      //   url: 'http://192.168.3.117:9090/jz/admin/getdetail/'+data,
      //   headers: {
    ...
    //手机号转换
    transForm(val){
      let aa = val.replace(/(\d{3})\d{4}(\d{4})/, '$1****$2')
      return aa
    }
  },
  //实例前
