        ...
        <el-collapse-item>
          ...
          <el-form @keyup.enter.native="fromShow" ref="from" :inline="true" :model="from" class="demo-form-inline">
            <el-form-item style="display: block">
              <div class="buttonDiv">
                <el-button class="mrr" :loading="allocationLoading" @click="distribution " size="small" type="success">
                  分配数据
                </el-button>

                <el-button class="mrr" :loading="recoverLoading" size="small" @click="manualRecycling" type="primary">
                  手动回收
                </el-button>
                <el-button class="mrr" size="small" @click="exportFile" type="success">导出数据</el-button>
                <el-button class="mrr" :loading="lostLoading" size="small" @click="activation" type="danger">战败激活
                </el-button>
...
<!--                clearable></el-cascader>-->
            ...
            <el-form-item prop="reverseSF">
              <el-checkbox @change="SFreverse" v-model="from.reverseSF">省份反选</el-checkbox>
            </el-form-item><!-- 省份反选 -->
...
<!--                clearable></el-cascader>-->
            ...
            <el-form-item prop="reverseCS">
              <el-checkbox @change="CSreverse" v-model="from.reverseCS">城市反选</el-checkbox>
            </el-form-item><!-- 城市反选 -->
            ...
            <el-form-item prop="addressOfNumber">
              <el-input
                size="small"
                placeholder="号码归属地"
                v-model="from.addressOfNumber">
                <i slot="prefix" class="el-input__icon el-icon-search"></i>
              </el-input>
            </el-form-item><!-- 号码归属地 -->
            <el-form-item prop="toAc">
              <el-checkbox border size="small" v-model="from.toAc">新车</el-checkbox>
            </el-form-item><!-- 新车 -->
            <el-form-item prop="toTt">
              <el-checkbox border size="small" v-model="from.toTt">检测</el-checkbox>
            </el-form-item><!-- 检测 -->
            <el-form-item prop="acResult">
              <el-checkbox border size="small" v-model="from.acResult">已传至爱弛</el-checkbox>
            </el-form-item><!-- 已传至爱弛 -->
            <el-form-item prop="resultId">
              <el-input
                size="small"
                placeholder="报名返回值id"
                v-model="from.resultId">
                <i slot="prefix" class="el-input__icon el-icon-search"></i>
              </el-input>
            </el-form-item><!-- 报名返回值id -->
            ...
            <el-form-item prop="queryCreateTime">
              <el-input
                size="small"
                placeholder="报名创建时间"
                v-model="from.queryCreateTime">
                <i slot="prefix" class="el-input__icon el-icon-search"></i>
              </el-input>
            </el-form-item><!--  报名创建时间 -->
            <el-form-item prop="queryInvite">
              <el-input
                size="small"
                placeholder="邀约状态"
            ...
            <el-form-item>
              <div class="buttonDiv">
                <el-button style="margin-right: 10px;" size="small" @click="fromShow" type="primary"
                           icon="el-icon-search">搜索
                </el-button>
                <el-button size="small" @click="fromClear('from')" type="info">清空<i
          ...
          <div style="background: rgb(245,245,245)">
            <div class="flex" style="width: 100%;">
              ...
              <div class="zux" style="width: 25%;">
                ...
                <div>
                  <span>第二名 : {{
                      this.second ? this.second.seatName : '无'
                    }}  </span><span>成功下单 : {{ this.second ? this.second.successAmount : '0' }}单</span>
                </div>
                <div>
                  <span>第三名 : {{
                      this.third ? this.third.seatName : '无'
                    }}  </span><span>成功下单 : {{ this.third ? this.third.successAmount : '0' }}单</span>
                </div>
                <div>
              ...
              <div>
                <el-form class="tablewb" :label-position="titleLeft" :inline="true" size="mini" ref="form" :model="form"
                  ...
                  <el-form-item style="width: 20%;" label="工号">
                    <el-select v-model="form.jobNumber"
                               placeholder="工号">
                      <el-option
                        v-for="item in optionsZx"
                        :key="item.value"
                  ...
                  <el-form-item style="width: 23%;" label="车辆品牌">
                    <el-input placeholder="车辆品牌" v-model="form.brand"></el-input>
<!--                    <el-select @change="carSeriesList" v-model="form.brand" placeholder="车辆品牌">-->
<!--                      <el-option-->
...
<!--                    </el-select>-->
                  ...
                  <el-form-item style="width: 23%;" label="车系">
                    <el-input placeholder="车系" v-model="form.family"></el-input>
<!--                    <el-select v-model="form.family" placeholder="车系">-->
<!--                      <el-option-->
...
<!--                    </el-select>-->
            ...
            <div class="wb xtxxDiv">
              ...
              <div class="mBottom">
                <el-checkbox border size="small" v-model="form.toAc">新车</el-checkbox>
                <el-checkbox border size="small" v-model="form.toTt">检测</el-checkbox>
              </div>
              <!--          <div>-->
              <!--            <span>历史备注：</span>-->
              <!--            <el-input-->
    ...
    <!--    上传数据填写自动外呼描述-->
    <el-dialog title="自动外呼描述" :visible.sync="dialogDescribe2">
      <el-input @input="disDisabled" placeholder="自动外呼描述" v-model="describe2"></el-input>
      <div slot="footer" class="dialog-footer">
        <el-button size="small" style="margin-right: 10px;" @click="describeShow2">取 消</el-button>
        <el-upload
          class="upload-demo mrr"
          :action=" this.baseURL+'/carac/readdata/'+this.describe2"
          :headers="headers"
  ...
  data() {
    return {
      ...
      from: {
        ...
        addressOfNumber:'',//号码归属地
        resultId:'',//报名返回值id
        error:'',//报名是否出错
        errorMessage:'',//交互错误原因
        queryCreateTime:'',//报名创建时间
        queryInvite:'', //邀约状态
  ...
  methods: {
    ...
    CSreverse(val){
      if(val){
        ...
        for (let i = 0; i < this.city.length; i++) {
          a.push(this.city[i].value)
        }

        let arr1 = this.from.city;
        let arr2 = a;
    ...
    SFreverse(val){
      if(val){
        ...
        for (let i = 0; i < this.province.length; i++) {
          a.push(this.province[i].value)
        }

        let arr1 = this.from.province;
        let arr2 = a;
        let list = arr2.filter(items => {
          if (!arr1.includes(items)) return items;
        })
        console.log(list);
        this.from.province = list
        this.spawnCity(this.from.province)
      }
    },
    ...
    spawnCity(val){
      console.log(val)
        axios({
          method: 'post',
          url: `${this.baseURL}/carac/getCarSeriesByBrand`,
          headers: {
          ...
          for (let i = 0; i < res.data.data.dataList.length; i++) {
            let a = {}
            a.label = res.data.data.dataList[i]
            a.value = res.data.data.dataList[i]
            this.city.push(a)
          }
        }).catch(err=>{
          console.log(err)
    ...
    getProvince(){
      axios({
        method: 'get',
        url: `${this.baseURL}/carac/getallprovinces`,
        headers: {
          "Content-Type": "application/json;charset=UTF-8",
          'token': localStorage.token,
        },
    ...
    licensePlate() {
      axios({
        method: 'get',
        url: `${this.baseURL}/carac/getbrands`,
        headers: {
          "Content-Type": "application/json;charset=UTF-8",
          'token': localStorage.token,
        },
    ...
    carSeriesList(val) {
      axios({
        method: 'get',
        url: `${this.baseURL}/carac/getCarSeriesByBrand/${val}`,
        headers: {
          "Content-Type": "application/json;charset=UTF-8",
          'token': localStorage.token,
        },
    ...
    exportFile() {
      this.$confirm('是否确认下载文件', '提示', {
        ...
        axios({
          method: 'post',
          url: this.baseURL + '/carac/exportalldatabycondition',
          headers: {
            "Content-Type": "application/json;charset=UTF-8",
            'token': localStorage.token,
          },
    ...
    activation() { //
      if (this.guids.length > 0) {
        ...
        axios({
          method: 'post',
          url: this.baseURL + '/carac/defeatactivationbyguidlist',
          headers: {
            "Content-Type": "application/json;charset=UTF-8",
            'token': localStorage.token,
          },
    ...
    manualRecycling() { //
      if (this.guids.length > 0) {
        ...
        axios({
          method: 'post',
          url: this.baseURL + '/carac/recoverydatabyguidlist',
          headers: {
            "Content-Type": "application/json;charset=UTF-8",
            'token': localStorage.token,
          },
    ...
    choiceZx() {
      ...
      axios({
        method: 'post',
        url: `${this.baseURL}/carac/allocatedatabyguids/${this.oneZx}/${this.readable}`,
        headers: {
          "Content-Type": "application/json;charset=UTF-8",
          'token': localStorage.token,
        },
    ...
    dataList(data) {
      ...
      axios({
        method: 'post',
        url: `${this.baseURL}/carac/getalldatabyconditition`,
        headers: {
          "Content-Type": "application/json;charset=UTF-8",
          'token': localStorage.token,
        },
        ...
        for ( let a = 0; a < res.data.data.dataList.length; a++) {
          if(res.data.data.dataList[a].toAc == true){
            res.data.data.dataList[a].toAc = '✔'
          }else if(res.data.data.dataList[a].toAc == false){
            res.data.data.dataList[a].toAc = '✘'
          }

          if(res.data.data.dataList[a].toTt == true){
            res.data.data.dataList[a].toTt = '✔'
          }else if(res.data.data.dataList[a].toTt == false){
            res.data.data.dataList[a].toTt = '✘'
          }

          if(res.data.data.dataList[a].acResult == true){
            res.data.data.dataList[a].acResult = '✔'
          }else if(res.data.data.dataList[a].acResult == false){
            res.data.data.dataList[a].acResult = '✘'
          }
    ...
    record(val) {
      ...
      axios({
        method: 'get',
        url: this.baseURL + '/carac/record/getRecordByCarRecordGuid?entityGuid=' + val.guid,
        headers: {
          "Content-Type": "application/json;charset=UTF-8",
          'token': localStorage.token,
        },
    ...
    ranking() {
      axios({
        method: 'post',
        url: this.baseURL + '/carac/gethetaoperformanceranking',
        headers: {
          "Content-Type": "application/json;charset=UTF-8",
          'token': localStorage.token,
        },
    ...
    get(data) {
      ...
      axios({
        method: 'get',
        url: this.baseURL + '/carac/getdetail/' + data,
        headers: {
          "Content-Type": "application/json;charset=UTF-8",
          'token': localStorage.token,
        },
    ...
    alldata() {
      ...
      axios({
        method: 'post',
        url: this.baseURL + '/carac/addstatus',
        headers: {
          "Content-Type": "application/json;charset=UTF-8",
          'token': localStorage.token,
        },
    ...
    modify(value, guid, a) {
      ...
      axios({
        method: 'get',
        url: `${this.baseURL}/carac/updatebyguidandfieldnameandvalue/${guid}/${a}/${value}`,
        headers: {
          "Content-Type": "application/json;charset=UTF-8",
          'token': localStorage.token,
        },
    ...
    swop() {
      ...
      axios({
        method: 'get',
        url: `${this.baseURL}/carac/primaryandstandbydataexchange?guid=${this.form.guid}&sparePhone=${this.form.sparePhone}&spareName=${this.form.spareName}`,
        headers: {
          "Content-Type": "application/json;charset=UTF-8",
          'token': localStorage.token,
        },
    ...
    branch() {
      axios({
        method: 'get',
        url: `${this.baseURL}/user/lockdepartment/${9}`,
        headers: {
          "Content-Type": "application/json;charset=UTF-8",
          'token': localStorage.token,
        },
