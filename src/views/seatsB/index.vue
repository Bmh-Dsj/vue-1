    ...
    <div class="right" v-loading="loading2">
          ...
          <div style="width:75%;">
            <div>
              <el-steps simple style="width:100%;font-size: 12px" space="10" :active="active" finish-status="success">
                <el-step title="邀约成功未添加微信"></el-step>
            ...
            <div class="right-s" style="width:100%; border-bottom: 1px #666 solid;background: rgba(245,245,245,0.5)">
              ...
              <div class="right-sd"><span :title="this.onedata.guid">数据编号：</span><span>{{ this.onedata.guid }}</span>
              </div>
              ...
              <div class="right-sd"><span :title="this.onedata.phone">联系方式：</span><span>{{ this.onedata.phone }}</span>
              </div>
              <div class="right-sd"><span
                :title="this.onedata.addressOfNumber">号码归属地：</span><span>{{ this.onedata.addressOfNumber }}</span></div>
      ...
      <div class="right-d">
        ...
        <div class="wb xtxxDiv">
          ...
          <div class="mBottom">
            <span class="required">添加微信通过：</span>
        ...
        <div class="czjlDid">
          <div class="record">
            ...
            <!--            <audio style="height: 26px;margin-top: 5px;" controls>-->
            ...
            <p v-for="a in record">
              {{ a.details }}
              <audio v-if="a.showudio" style="outline: none;height: 25px;" controls="controls" :src="a.audio"></audio>
            </p>
          ...
          <div class="buttonsDiv">
            <div class="buttonDiv">
              ...
              <!--              </el-button>-->
              <el-button @click="dialNumber" size="small" type="success">拨打电话
                <i class="el-icon-phone-outline el-icon--right"></i>
              </el-button>
            </div>
            <div class="buttonDiv">
              <el-button @click="hangUpThePhone" size="small" type="danger">挂断电话
                <i class="el-icon-phone el-icon--right"></i>
              </el-button>
            </div>
            <div class="buttonDiv">
              <el-button :loading="loadingT" @click="alldata" size="small" type="primary">提交任务
                <i class="el-icon-s-claim el-icon--right"></i>
              </el-button>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    getlist(data) {
      this.loading = true
      this.loading2 = true
      //   /hetao/admin/getneedhandledatatob
      // this.$store.dispatch('user/htgdList',data).then(res=>{
      axios({
        method: 'post',
        url: `${this.baseURL}/hetao/admin/getneedhandledatatob`,
        headers: {
          "Content-Type": "application/json;",
          'token': localStorage.token,
        },
    ...
    get(data) {
      ...
      this.$store.dispatch('user/htgdoneList', data).then(res => {
        ...
        //audio
        for (let i = 0; i < res.data.data.record.length; i++) {
          if (res.data.data.record[i].audio == '') {
            res.data.data.record[i].showAudio = false
          } else {
            res.data.data.record[i].showAudio = true
          }
        }
        this.record = res.data.data.record
        this.ranking()//排名
